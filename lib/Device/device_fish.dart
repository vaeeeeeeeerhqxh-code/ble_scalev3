...
class DeviceFish extends StatefulWidget {
  final PPDeviceModel device;

  const DeviceFish({Key? key, required this.device}) : super(key: key);

  @override
  State<DeviceFish> createState() => _DeviceFishState();
}

class _DeviceFishState extends State<DeviceFish> {
  ...
  Future<void> _handle(String title) async {
    ...
    try {
      ...
      if (title == DeviceMenuType.toZero.value) {
        _updateText('toZero');
        final ret = PPPeripheralFish.toZero().timeout(const Duration(seconds: 3));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Fish')),
      body: Column(
        children: [
          Container(
